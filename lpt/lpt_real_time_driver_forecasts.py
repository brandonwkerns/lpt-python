...
def lpt_real_time_driver_forecasts(dataset,plotting,output,lpo_options,lpt_options,merge_split_options,argv):
    ...
    if options['do_lpt_calc']:
        ...
        LPT0, BRANCHES0 = lpt.helpers.init_lpt_group_array(dt_list, options['objdir'], fmt=fmt)
        ## Remove small LPOs
        ...
        if merge_split_options['allow_merge_split']:
            LPT, BRANCHES = lpt.helpers.lpt_split_and_merge(LPT_remove_short, BRANCHES_remove_short, merge_split_options, options, fmt=fmt)
        else:
        ...
        if merge_split_options['allow_merge_split']:
            TIMECLUSTERS = lpt.helpers.calc_lpt_system_group_properties_with_branches(LPT, BRANCHES, options, fmt=fmt)
        else:
            TIMECLUSTERS = lpt.helpers.calc_lpt_system_group_properties(LPT, options, fmt=fmt)
