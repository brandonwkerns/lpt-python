...
from lpt_real_time_driver_forecasts import *

plt.close('all')
plt.ioff()

"""
**** Real-time LPT driver script for CFS Realtime Forecast data. ****

**** This is set up to use the first 45 day forecast period. ****
If no command line arguements, it uses the latest data closest to real time.
The initialization date can be specified on the command line.

Example usage:
   python lpt_real_time_driver__cfs_forecast.py
   python lpt_real_time_driver__cfs_forecast.py 2019040100
...
dataset['sub_directory_format'] = 'cfs.%Y%m%d/%H/time_grib_01'
dataset['forecast_hours_interval'] = 6           # Time resolution of the data in hours.
