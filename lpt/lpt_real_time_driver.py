...
## This driver script is for real time analysis or data
##   It is for going backwards from a specified time (e.g., near real time).
...
def lpt_real_time_driver(dataset,plotting,output,lpo_options,lpt_options,merge_split_options,argv):
    ...
    ## Check back "hours_to_go_back" h from current time.
    for hours_back in range(0, hours_to_go_back+1, dataset['data_time_interval']):
        ...
        try:
            ...
            if lpo_options['do_lpo_calc']:
                ...
                if os.path.exists(objects_fn):
                    print('This time already has LPO step done.')
                    continue
    ...
    if options['do_lpt_calc']:
        ...
        lpt.lptio.lpt_systems_group_array_output_ascii('test1.txt', LPTfb, BRANCHESfb)
